...
def _candidates() -> list[GateMessage]:
    return [
        GateMessage(
            message_id="1", author_display="alice", content="how do decorators work?"
        ),
        GateMessage(message_id="2", author_display="bob", content="what's for lunch?"),
        GateMessage(
            message_id="3", author_display="cara", content="explain async/await"
        ),
    ]


def _answering(
    verdicts: dict[str, bool], captured: dict | None = None
) -> FunctionModel:
...
async def test_asks_one_bool_question_per_candidate():
    ...
    _use(
        _answering(
            {"candidate_1": True, "candidate_2": True, "candidate_3": True}, captured
        )
    )
...
@pytest.mark.asyncio
async def test_cancellation_propagates_instead_of_failing_open():
    started = asyncio.Event()

    async def _hang(messages, info: AgentInfo):
        started.set()
        await asyncio.sleep(60)
        raise AssertionError("the gate should have been cancelled first")

    _use(FunctionModel(_hang))
    task = asyncio.create_task(
        filter_messages("only python questions", _candidates(), [])
    )
...
async def test_grounding_is_rendered_but_never_judged():
    grounding = [
        GateMessage(message_id="g1", author_display="dan", content="earlier chatter")
    ]
    captured: dict = {}
    _use(
        _answering(
            {"candidate_1": True, "candidate_2": False, "candidate_3": False}, captured
        )
    )
