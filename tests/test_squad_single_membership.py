...
async def test_user_single_squad_membership():
    ...
    try:
        ...
        # Get user's bytes balance before switching squads
        ...
        # Verify bytes were deducted for switching squads
...
@pytest.mark.asyncio
async def test_user_insufficient_bytes_for_squad_switch():
    """Test that a user cannot switch squads if they don't have enough bytes."""
    # Create a test client
    client = TestClient(app)
    ...
    try:
        ...
        # Create bytes config with high squad switch cost
        bytes_config = BytesConfig(
            guild_id=test_guild.id,
            squad_join_bytes_required=0,  # No bytes required for testing
            squad_switch_cost=200  # Cost to switch squads (higher than user's balance)
        )
        session.add(bytes_config)
        session.commit()

        # Create test user with low bytes balance
        test_user = DiscordUser(
            discord_id=123456789,
            username="test_user",
            bytes_balance=100  # Not enough to switch squads
        )
        session.add(test_user)
        session.commit()
        ...
        # Get user's bytes balance before attempting to switch squads
        initial_balance = test_user.bytes_balance
        assert initial_balance == 100

        # Try to add user to second squad (this should fail due to insufficient bytes)
        response = client.post(
            f"/api/squads/{squad2.id}/members",
            json={"user_id": test_user.discord_id},
            headers=headers
        ...
        assert "bytes" in error_data["error"].lower()  # Error message should mention bytes

        # Verify user is still in first squad
        response = client.get(
            f"/api/users/{test_user.discord_id}/squads",
            headers=headers
        )
        ...
        # Verify bytes were not deducted
        assert test_user.bytes_balance == initial_balance
        assert test_user.bytes_balance == 100  # Still 100
