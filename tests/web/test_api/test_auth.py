...
class TestAuthHealthCheck:
    ...
    async def test_health_check_success(self, real_api_client: AsyncClient):
        ...
        response = await real_api_client.get("/auth/health")
...
class TestAuthErrors:
    ...
    async def test_case_sensitive_bearer(self, real_api_client: AsyncClient):
        ...
        # HTTPBearer accepts case-insensitive "bearer", but verify_api_key
        # checks the raw header for case-sensitive "Bearer " prefix and
        # returns 401 if mismatched
        assert response.status_code == 401
...
class TestAuthIntegration:
    ...
    async def test_auth_flow_complete(
        ...
        validate_response = await real_api_client.post(
            "/auth/validate",
            headers=bot_headers
        )
        ...
        # Then check status
        status_response = await real_api_client.get(
            "/auth/status",
            headers=bot_headers
        )
        ...
        health_response = await real_api_client.get("/auth/health")
    ...
    async def test_concurrent_auth_requests(
        ...
        tasks = [
            real_api_client.post("/auth/validate", headers=bot_headers)
            for _ in range(5)
        ]
