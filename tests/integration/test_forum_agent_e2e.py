...
class TestForumAgentEndToEnd:
    ...
    async def test_discord_forum_post_to_database_pipeline(self, real_db_session):
        ...
        # Instead of using the real bot (which has a read-only .rest property),
        # test the forum agent service processing directly
        ...
        with patch.object(APIClient, '__init__', return_value=None), \
            ...
            with patch('smarter_dev.bot.agents.forum_agent.ForumMonitorAgent.evaluate_post') as mock_evaluate:
                mock_evaluate.return_value = (
                    "The post contains a clear question that matches my expertise.",
                    0.85,
                    "This is a helpful response generated by the AI agent.",
                    250
                )
                ...
                post_data = ForumPostData(
                    ...
                    content="I'm new to programming and want to learn. Where should I start?",
                    author_display_name="NewCoder",
                    tags=["programming", "beginner"],
                    attachments=[],
                    channel_id="forum_123",
                    thread_id="987654321",
                    guild_id=guild_id
                )
        ...
        # Step 3: Verify the processing
        assert len(responses) == 1
        response = responses[0]
        assert response["agent_name"] == "E2E Test Agent"
        assert response["should_respond"] is True
        assert response["confidence"] == 0.85
        assert "helpful response generated" in response["response_content"]
    ...
    async def test_multiple_agents_concurrent_processing(self, real_db_session):
        ...
        with patch.object(APIClient, '__init__', return_value=None), \
            ...
            def mock_get_side_effect(url, **kwargs):
                if "count" in url:
                    return _mock_response({"count": 0})  # No recent responses
                return _mock_response(agents_list)
            ...
            def mock_ai_evaluation(system_prompt, *args, **kwargs):
                ...
                elif "web development" in system_prompt.lower():
    ...
    async def test_error_resilience_and_recovery(self, real_db_session):
        ...
        with patch.object(APIClient, '__init__', return_value=None), \
            ...
            with patch('smarter_dev.bot.agents.forum_agent.ForumMonitorAgent.evaluate_post') as mock_evaluate:
                ...
                # First post - AI fails, should handle gracefully (error caught per-agent)
                responses_1 = await service.process_forum_post(guild_id, post_data)
                # The agent error is caught per-agent, so we get an empty list
                assert isinstance(responses_1, list)
                assert len(responses_1) == 0  # Error caught, no response generated
    ...
    async def test_real_time_configuration_updates(self, real_db_session):
        ...
        with patch.object(APIClient, '__init__', return_value=None), \
            ...
            # Mock rate limit and response recording
            def mock_get_side_effect_1(url, **kwargs):
                if "count" in url:
                    return _mock_response({"count": 0})
                return _mock_response(agents_list_high_threshold)

            mock_get.side_effect = mock_get_side_effect_1
            mock_post.return_value = _mock_response({"id": str(uuid4())})
        ...
        with patch.object(APIClient, '__init__', return_value=None), \
            ...
            def mock_get_side_effect_2(url, **kwargs):
                if "count" in url:
                    return _mock_response({"count": 0})
                return _mock_response(agents_list_low_threshold)

            mock_get.side_effect = mock_get_side_effect_2
            mock_post.return_value = _mock_response({"id": str(uuid4())})
        ...
        with patch.object(APIClient, '__init__', return_value=None), \
            ...
            mock_get.return_value = _mock_response(agents_list_different_forum)
