...
class TestE2EScenarios:
    ...
    async def test_complete_bytes_workflow(self, real_db_session, real_api_client, bot_headers, test_guild_id, test_user_id, test_user_id_2):
        ...
        if response.status_code == 409:
            ...
            assert claim_data["balance"]["balance"] >= initial_balance  # Balance should not decrease
        ...
        if response.status_code == 409:
            ...
            import pytest
            pytest.skip(f"Transfer failed with 409 - likely insufficient balance: {response.json()}")
        else:
            assert response.status_code == 200
        ...
        assert giver_balance["balance"] >= 0, f"Giver balance should be non-negative: {giver_balance['balance']}"
        ...
        assert receiver_balance["balance"] >= 0, f"Receiver balance should be non-negative: {receiver_balance['balance']}"

    async def test_complete_squad_workflow(self, real_db_session, real_api_client, bot_headers, test_guild_id, test_user_id, test_role_id):
    ...
    async def test_service_layer_integration(self, real_db_session, real_api_client, bot_headers, api_settings, test_guild_id, test_user_id):
    ...
    async def test_error_handling_scenarios(self, real_db_session, real_api_client, bot_headers, test_guild_id, test_user_id):
        ...
        assert response.status_code in [400, 409, 500], f"Expected 400, 409, or 500, got {response.status_code}"
    ...
    async def test_concurrent_operations(self, real_db_session, real_api_client, bot_headers, test_guild_id, test_user_id):
        ...
        try:
            ...
            assert len(successful_responses) >= 1, "At least one concurrent operation should succeed"
    ...
    async def test_data_consistency(self, real_db_session, real_api_client, bot_headers, test_guild_id, test_user_id, test_user_id_2):
        ...
        if response2.status_code == 409:
            ...
            import pytest
            pytest.skip(f"Transfer failed with 409 - likely insufficient balance or cooldown: {response2.json()}")
        ...
        assert final_balance >= expected_balance  # Should be at least the expected balance

    async def test_cache_invalidation(self, real_db_session, real_api_client, bot_headers, test_guild_id, test_user_id):
        ...
        if expected_balance_increase:
            assert balance2 > balance1, f"Balance should have increased from {balance1} to {balance2}"
        else:
            # Balance should be the same if daily claim was already used
            assert balance2 >= balance1, f"Balance should not have decreased from {balance1} to {balance2}"
        ...
        assert balance3 == balance2, f"Balance should remain consistent: {balance2} vs {balance3}"