...
from smarter_dev.bot.utils.embeds import create_leaderboard_embed
...
async def send_command(ctx: lightbulb.Context) -> None:
    ...
    if amount < 1 or amount > 10000:
        await respond_with_error_card(
            ctx.respond,
            "Amount must be between 1 and 10,000 bytes.",
            flags=hikari.MessageFlag.EPHEMERAL,
        )
        return
    ...
    try:
        ...
        if user.id == ctx.user.id:
            await respond_with_error_card(
                ctx.respond,
                "You can't send bytes to yourself!",
                flags=hikari.MessageFlag.EPHEMERAL,
            )
            return
        ...
        await respond_with_error_card(
            ctx.respond,
            f"Insufficient balance! You need {e.required:,} bytes but only have {e.available:,}.",
            flags=hikari.MessageFlag.EPHEMERAL,
        )
        return
...
async def leaderboard_command(ctx: lightbulb.Context) -> None:
    ...
    try:
        ...
        await respond_with_error_card(
            ctx.respond,
            "Failed to get leaderboard. Please try again later.",
            flags=hikari.MessageFlag.EPHEMERAL,
        )
    except Exception as e:
...
async def history_command(ctx: lightbulb.Context) -> None:
    ...
    try:
        ...
        await respond_with_error_card(
            ctx.respond,
            "Failed to get transaction history. Please try again later.",
            flags=hikari.MessageFlag.EPHEMERAL,
        )
    except Exception as e:
...
async def info_command(ctx: lightbulb.Context) -> None:
    ...
    try:
        ...
        await respond_with_error_card(
            ctx.respond,
            "Failed to get configuration. Please try again later.",
            flags=hikari.MessageFlag.EPHEMERAL,
        )
    except Exception as e:
...
async def send_bytes_context_menu(ctx: lightbulb.Context) -> None:
    ...
    if recipient.id == ctx.user.id:
        await respond_with_error_card(
            ctx.respond,
            "You cannot send bytes to yourself!",
            flags=hikari.MessageFlag.EPHEMERAL,
        )
        return
    ...
    try:
        ...
        await respond_with_error_card(
            ctx.respond,
            "Unable to verify user membership in this server.",
            flags=hikari.MessageFlag.EPHEMERAL,
        )
        return
    ...
    from smarter_dev.bot.views.bytes_views import SendBytesModalHandler
    from smarter_dev.bot.views.bytes_views import create_send_bytes_modal
