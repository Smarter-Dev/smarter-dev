...
class ConversationContextBuilder:
    ...
    def __init__(self, bot: hikari.GatewayBot, guild_id: int | None = None):
        self.bot = bot
    ...
    async def _build_conversation_timeline(self, messages: list[hikari.Message], trigger_message_id: int | None, users_by_id: dict[str, dict]) -> str:
    ...
    def _generate_conversation_summary(self, messages: list[hikari.Message], users_by_id: dict[str, dict]) -> str:
        ...
        recent_messages = [m for m in messages if (datetime.now(UTC) - m.created_at).total_seconds() < 300]
        if recent_messages:
    ...
    async def _build_user_info(self, user_id: int) -> dict[str, Any] | None:
