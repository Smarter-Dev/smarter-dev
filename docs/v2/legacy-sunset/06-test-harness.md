...
migration. It boots the **whole app** (Skrift, with the native `/api`
controllers) against throwaway podman postgres/redis, seeds representative
data into the single database, and asserts every bot-consumed API endpoint
and every admin page still works. Verification gates run it before every
phase transition.
...
| Piece | Detail |
| --- | --- |
| postgres | podman `postgres:15-alpine` on port **55432**, container `smarter_dev_harness_postgres`, no named volume. Init scripts from `scripts/postgres-init/` create the single `smarter_dev` database with its `skrift` schema — the same layout as prod. |
| redis | podman `redis:7-alpine` on port **56379**, container `smarter_dev_harness_redis`. |
| migrations | `scripts/migrate.py` (both trees: skrift-main, main) run as subprocesses with `DATABASE_URL` pointed at the harness DB. |
| seed | `scripts/local_harness/seed.py` — since the phase-02 DB consolidation everything seeds into the **main DB's `skrift` schema**: the adopted bot tables (bytes config/balances/transactions, squads + memberships, sale events, forum agents + responses + notification topics + subscriptions, campaigns + challenges, scheduled + repeating messages, audit-log/AoC/attachment-filter configs, help conversations, channel model overrides) plus quests + daily quest, feature flags, chat-agent engagement, member activity, channel handler, Skrift setup-complete settings, and a **known-plaintext Skrift-native `sk_` service API key** in `skrift.api_keys` owned by a `bot@smarter.dev` service user. Only `sk_` keys exist — the legacy key system was deleted in phase 05. |
| app | `hypercorn scripts.local_harness.harness_app:app` on port **8791** with `SKRIFT_ENV=development`. `harness_app` is `main:app` plus one patch: the admin Discord client's base URL is pointed at the local mock Discord server. |
| mock Discord | `scripts/local_harness/mock_discord.py` on port **8792** — serves the guild/roles/channels fixtures the Skrift bot-admin pages fetch, so every page renders fully offline. |
...
1. **`API_CHECKS`** — every bot-consumed `/api` endpoint group with Bearer
   ...
   key), malformed/missing/unknown-`sk_` key → 401/403,
...
3. **`LEGACY_ADMIN_PAGES`** — the removed legacy `/bot-admin` tree is
   asserted absent (404) with the anonymous client.
4. **`UNAUTHENTICATED_PAGES`** — anonymous requests to `/admin/` must NOT
   return 200.
...
- **New API key format** (Skrift-native `sk_` keys): done — only
  ...
  authenticates. Phase 05 deleted the legacy `sk-` key system entirely; the
  remaining malformed/unknown-key 401 checks pin the `sk_` scheme.
...
- **Single database**: done — all seeds land in the single database's
  `skrift` schema. Phase 05 deleted `LEGACY_DATABASE_URL`, the harness's
  second database, and the `alembic/legacy` tree.
