...
- **The mod-command suite is mostly superseded — by native Discord moderation, not by custom slash commands.** `/warn` and `/timeout` already exist as slash plugins writing `ModerationAction` rows; the Muted-role system is obsoleted by native timeouts. The interactive act of banning/kicking/timing-out is covered by Discord's own right-click moderation UI (Kick/Ban/Timeout/Remove Timeout, all with reasons) — so we do **not** build `/ban`, `/kick`, or a `/timeout remove` command (decision 2026-07-18). Instead a bot-core **audit-log ingestion listener** records those native actions as `ModerationAction(source="audit_log")` rows, which fire the `mod_action` trigger and are visible to `list_mod_actions`, so handlers see native moderation like any other action. What remains bot-core is `/purge` (no native bulk-delete equivalent) plus completing the ingestion listener. Handler-side *read views* (`!history`/`!whois`/`!lookup` re-imagined as handlers in the mod channel) still need metered read functions over `ModerationAction` and member data — the same reads the rejoin alert needs.
- **One audit system.** Native kick/ban/unban/timeout land as `ModerationAction(source="audit_log")` rows via the ingestion listener; the surviving manual slash commands (`/warn`, `/timeout`, `/purge`) write `source="manual"`; the auto-mod handler's actions get recorded with a new `source="handler"`; the moderation triage agent already writes `source="ai"`. A new `mod_action` trigger lets a handler own mod-log-channel formatting for all four.
...
| `!ban` (alias `!boop`) | drop (superseded by native moderation + audit-log ingestion) — 2026-07-18 | Discord's native right-click Ban (with reason) covers the interactive act; the audit trail is captured by the ingestion listener as `ModerationAction(source="audit_log")`, which fires the `mod_action` trigger and is visible to `list_mod_actions` — so `!history`, the rejoin alert, and the mod-log formatter see native bans like any other action. Accepted loss vs a custom `/ban`: no DM-the-user-before-action courtesy notice. |
| `!kick` | drop (superseded by native moderation + audit-log ingestion) — 2026-07-18 | Same rationale as `!ban`: native right-click Kick + ingestion (`source="audit_log"`, via the audit-log fetch on member leave). Accepted loss: no pre-action courtesy DM. |
...
| `!unmute` | drop (superseded by native moderation + audit-log ingestion) — 2026-07-18 | Superseded by Discord's native Remove Timeout (right-click); we do **not** add a `/timeout remove` command (earlier plan reversed 2026-07-18). The ingestion listener records the removal as `ModerationAction(action_type="untimeout", source="audit_log")` — the completion work adds untimeout recording (§4.2). |
...
*Design.* Gateway listeners in `client.py` observe the native events and, for actions the **bot itself did not perform**, fetch the responsible moderator + reason from Discord's audit log and write the `ModerationAction` row:

- **Kick** — no dedicated gateway event, so it is detected on `MemberDeleteEvent` (member leave) by fetching the most recent `MEMBER_KICK` audit-log entry for that user within a short recency window; a voluntary leave produces no matching entry and records nothing.
- **Ban** — `BanCreateEvent` → `MEMBER_BAN_ADD` audit-log entry.
- **Unban** — `BanDeleteEvent` → `MEMBER_BAN_REMOVE` audit-log entry.
- **Timeout** — `MemberUpdateEvent` where `communication_disabled_until` transitions to a non-null future time → `MEMBER_UPDATE` audit-log entry; duration derived from the new timeout expiry.

Every path excludes the bot's own actions (skips when the audit-log actor is the bot user, which `mod_tools`/handlers already record) and calls `dispatch_mod_action` after commit so the `mod_action` trigger fires.

*Status: partially landed; completion in progress on `feat/bot-core-parity`.* `smarter_dev/bot/audit_logger.py` already implements the kick, ban, unban, and timeout ingestion paths (audit-log fetch, bot-self exclusion, `source="audit_log"` row, `dispatch_mod_action`), and all four are wired into the `client.py` gateway listeners. **Completion work still owed:**

- **Untimeout (Remove Timeout) recording** — `log_member_update` currently records only when a timeout is *applied* (`communication_disabled_until` becomes non-null); the *removal* transition (non-null → null) is not recorded. Add an `action_type="untimeout"` path so native "Remove Timeout" lands as an audit row (this is the `!unmute` replacement).
- **Tests** — none of the ingestion paths are tested. Add coverage for each happy path (native kick/ban/unban/timeout/untimeout → correct `ModerationAction` row + `dispatch_mod_action` call) and the critical exclusions (bot-performed action → no row; voluntary leave with no kick entry → no row; audit-log fetch failure → logged, no crash).
- **No inline imports** — the `client.py` listeners and `audit_logger.py` currently import via inline `from smarter_dev.bot.audit_logger import …` inside the handlers; move these to module top per the project convention while wiring the remaining path.
...
**Handler F — `mod-log-formatter` (trigger: `mod_action`, guild-wide).** Formats every audit row into the mod-log channel: action, target, moderator (or "auto-mod"/"AI"), reason, duration, source. Replaces the legacy per-command embed posting and covers native kick/ban/unban/timeout (ingested as `source="audit_log"`), `/purge`, `/warn`, `/timeout`, AI triage, and the spam engine uniformly. Runs with `max_mod_actions=0` (hard rail, §3.5). Optional skip: `if context["source"] == "handler" and context["action_type"] == "timeout"` when the spam engine already posted its own escalation ping, to avoid double-posting — decided at authoring time.

**Dropped from py-moderation:** `!ban`/`!kick` (native right-click moderation + audit-log ingestion, 2026-07-18), `!mute`/`!unmute`/scheduled unmute (native timeouts + native Remove Timeout, recorded via untimeout ingestion), `!warn` (existing `/warn`), `ModAction` pickle table (existing `ModerationAction`), role-name gates with silent failure, cog error swallowing.
...
**Phase 1 — `/purge` + ingestion completion (independent, immediate value).**
`/purge` (the one surviving bot-core moderation command) plus completing the native-moderation audit-log ingestion listener (§4.2): add untimeout recording, add tests for every ingestion path, and move the inline `audit_logger` imports to module top.
Tests first — `/purge`: permission-denied ephemeral; purge per-user admin-override guard; >14-day-old message filtering; writes the correct `ModerationAction(action_type="purge", source="manual")` row; REST 403/404 mid-action fails loud with an ephemeral error and does **not** write an audit row for an action that didn't happen. Tests first — ingestion: native kick/ban/unban/timeout/untimeout each produce the correct `ModerationAction(source="audit_log")` row and call `dispatch_mod_action`; bot-performed actions produce no row; a voluntary leave with no matching kick audit-log entry produces no row; an audit-log fetch failure is logged without crashing the listener.
...
| `!mute` / `!unmute` / persisted unmute scheduler | Superseded by `/timeout` + native expiry; `!unmute` is superseded by native Remove Timeout, recorded via untimeout audit-log ingestion (no `/timeout remove` command — 2026-07-18). |
| `!ban` / `!kick` slash rebuilds | Reversed 2026-07-18: Discord's native right-click Ban/Kick (with reasons) covers the interactive act; the audit-log ingestion listener records them as `source="audit_log"`. Accepted loss: no pre-action courtesy DM to the target. |
