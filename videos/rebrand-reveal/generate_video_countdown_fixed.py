#!/usr/bin/env python3
"""
Smarter Dev Rebrand Reveal Video Generator - Fixed Countdown Transitions
Epic "Level Up" reveal video with smooth countdown transitions using correct MoviePy API.
"""
...
def generate_video():
    ...
    # Create enhanced countdown with smooth transitions
    print("🔢 Creating enhanced countdown with smooth transitions...")
    ...
    for i, number in enumerate([5, 4, 3, 2, 1]):
        ...
        try:
            ...
            # Add smooth transitions with correct API
            if i == 0:  # First number (5) - scale entrance
                text_clip = (text_clip
                            .with_position(('center', 'center'))
                            .with_start(start_time)
                            .with_duration(1.0)
                            .with_effects([vfx.fadein(0.2)])
                            .resized(lambda t: 0.5 + 0.5 * min(t/0.2, 1)))
            elif i < 4:  # Numbers 4, 3, 2 - pulse effect
                text_clip = (text_clip
                            .with_position(('center', 'center'))
                            .with_start(start_time)
                            .with_duration(1.0)
                            .with_effects([vfx.fadein(0.1), vfx.fadeout(0.2)])
                            .resized(lambda t: 1.0 + 0.1 * np.sin(t * 8)))  # Pulse effect
            else:  # Last number (1) - dramatic scale
                text_clip = (text_clip
                            .with_position(('center', 'center'))
                            .with_start(start_time)
                            .with_duration(1.0)
                            .with_effects([vfx.fadein(0.05), vfx.fadeout(0.4)])
                            .resized(lambda t: 0.8 + 0.4 * min(t/0.1, 1)))
            
            clips.append(text_clip)
            print(f"✅ Added countdown: {number} with smooth transition")
            
        except Exception as e:
            print(f"⚠️  Could not create countdown {number}: {e}")
            # Fallback without custom font but with basic transitions
            try:
                text_clip = TextClip(
                    text=str(number), 
                    font_size=200, 
                ...
                # Basic fade transition for fallback
                text_clip = (text_clip
                            .with_position(('center', 'center'))
                            .with_start(start_time)
                            .with_duration(1.0)
                ...
                clips.append(text_clip)
                print(f"✅ Added countdown: {number} (fallback with fade)")
            except Exception as e2:
                print(f"❌ Countdown {number} failed completely: {e2}")
    
    # Create "BEGINNER.CODES" glitch transition
    ...
    try:
        ...
        old_brand = (old_brand
                    ...
                    .with_duration(1.0)
                    .with_effects([vfx.fadein(0.1), vfx.fadeout(0.5)]))
    ...
    # Create brand reveal with smooth animations
    print("🧠 Creating brand reveal...")
    try:
        # Brain logo with scale entrance
        ...
        brain_clip = (brain_img
                     ...
                     .resized(lambda t: 0.8 + 0.2 * min(t/0.5, 1)))
        ...
        try:
            ...
            # Slide up effect using position function
            def slide_up_position(t):
                slide_offset = max(0, 30 * (0.4 - t) / 0.4)  # Slide from 30px below to final position
                return ('center', VIDEO_HEIGHT // 2 + 50 + slide_offset)
            
            smarter_text = (smarter_text
                           .with_position(slide_up_position)
        ...
        try:
            ...
            # Slide up effect for "dev"
            def slide_up_dev_position(t):
                slide_offset = max(0, 30 * (0.4 - t) / 0.4)
                return ('center', VIDEO_HEIGHT // 2 + 150 + slide_offset)
            
            dev_text = (dev_text
                       .with_position(slide_up_dev_position)
        ...
        try:
            ...
            subtitle = (subtitle
                       ...
                       .with_effects([vfx.fadein(1.0)]))  # Slow fade
    ...
    try:
        if (audio_path / "beep-1.mp3").exists():
            for i in range(5):  # 5 numbers: 5, 4, 3, 2, 1
                ...
                # Increase pitch slightly for each beep by changing playback speed
                speed_factor = 1.0 + (i * 0.1)  # 1.0, 1.1, 1.2, 1.3, 1.4
                beep = (AudioFileClip(str(audio_path / "beep-1.mp3"))
                       .subclipped(0, 0.8)
                       .with_start(beep_time)
                       .with_volume_scaled(0.8 + i * 0.04)  # Slightly increase volume each time
                       .with_effects([afx.speedx(speed_factor)]))  # Increase pitch
                audio_clips.append(beep)
            print("✅ Added countdown beeps (all using beep-1 with increasing pitch)")
    except Exception as e:
    ...
    # Export video
    output_file = output_path / "smarter_dev_rebrand_reveal_countdown_fixed.mp4"
    ...
    try:
        final_video.write_videofile(
            ...
            codec='libx264',
            audio_codec='aac',
            temp_audiofile='temp-audio-countdown-fixed.m4a',
            remove_temp=True
        )
